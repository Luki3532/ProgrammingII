...
class twoDArrays {
    ...
    //             The main method itterates through a 2D array and displays information and 
    //               prompts to the user

    // TEMP VARIABLES FOR PRIVATE METHODS
    private static int Temp = 0;                    // temporary variable for 
                                                    // sum of array method created

    private static int[] TempArray = new int[4];    // temporary array for column 

    public static void main(String[] args) {
        int[][] Array = new int[4][3];              // MAIN 2D Array created
                                                    // Row(4) X Column(3)

        Scanner Read = new Scanner(System.in);      // Instance of Scanner class for 
                                                    // Array population by user created
        ...
        for (int[] row : Array) {
            for (int value : row) {
                System.out.printf("%4d", value);
                                                    //Print appropriate element to satisfy 
                                                    // the 3x4 display of element
            }
            System.out.println();                   //Add empty line
        }
    ...
    System.out.println("The sum of elements in col 3 is: "+twoDArrays.sumOfElements(getColumnArray(Array, 3)) );
                                                    ...
                                                    //  GetColumArray to build a new 1D array 
    ...
    private static int sumOfElements(int[] Array) {
        ...
        for (int i = 0; i < Array.length; i++) {    //Parse through all elements of 
                                                    // 1D Array
    ...
    private static int[] getColumnArray(int[][] Array, int ColumnRequested){
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 3; j++) {
                if (j == ColumnRequested-1) {       //If the itterated column matches the 
                                                    // Actual parameter 'ColumnRequested' 
                                                    
                    TempArray[i] = Array[i][j];     //  then Append 'TempArray' with the 
                                                    // Respective Element 
                                                    //      -from the Actual 2D int 
                                                    //          parameter'Array'.    
                }
            }
        }
