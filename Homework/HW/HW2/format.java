...
class twoDArrays {
    //DESCRIPTION: This class has 2 methods that manipulate the 2D Array and 1 main method
    ...
    // temporary variable for sum of array method created

    private static int[] TempArray = new int[4];    
    // temporary array for column 

    public static void main(String[] args) {
        int[][] Array = new int[4][3];              
        // MAIN 2D Array created Row(4) X Column(3)

        Scanner Read = new Scanner(System.in);      
        // Instance of Scanner class for Array population by user created


        // Itterate through Array while prompting user to fill each element of the 2d array
        for (int i = 0; i < 4; i++) {  

            //itterate through all 4 rows
            for (int j = 0; j < 3; j++) {           
                //itterate through each element
                
            System.out.print("Enter a value for row " + (1+i) + " column " + (j+1) + " "); 
            //Prompt user for input and specify 
                                                    
            // input prompt for the row and column

                Array[i][j] = Read.nextInt();       
                //Scan for input in terminal
            }
        ...
        //From TwoDArray class --> SumOfElements() 
                                                    
                                                    
        // for the respective print statements.
    ...
    // From TwoDArray class use --> 
                                                    
    // GetColumArray to build a new 1D array to find the sum of elements (TwoDArray Private Method) for the respective columns.(1,2,3)
    }

    private static int sumOfElements(int[] Array) {
        //DESCRIPTION: add up all elements from actual array parameter 'int[] Array' then 
                                                    
        // returns result.
        Temp = 0;                                   
        //static variable is set to 0

        for (int i = 0; i < Array.length; i++) {    
            //Parse through all elements of 
                                                    
            // 1D Array

            Temp += Array[i];                       
            //Element in array is added to Result
        }
        return Temp;                                
        //Result is Returned
    }
    private static int[] getColumnArray(int[][] Array, int ColumnRequested){
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 3; j++) {
                if (j == ColumnRequested-1) {       
                    //If the itterated column matches the 
                                                    
                    // Actual parameter 'ColumnRequested' 
                                                    
                    TempArray[i] = Array[i][j];     
                    //  then Append 'TempArray' with the 
                                                    
                    // Respective Element -from the Actual 2D int parameter'Array'.    
                }
            }
        }
        return TempArray;                           
